...
Initialize with `node scripts/pib-db.js init`.
...
CLI: `node scripts/pib-db.js <command>`
...
**Direct SQL:**
- `node scripts/pib-db.js query "SELECT fid, text, status FROM actions WHERE completed = 0 AND deleted_at IS NULL ORDER BY project_fid, sort_order"`
- `node scripts/pib-db.js query "SELECT fid, name, status FROM projects WHERE deleted_at IS NULL"`
- `node scripts/pib-db.js query "SELECT fid, text, due FROM actions WHERE flagged = 1 AND completed = 0 AND deleted_at IS NULL"`
...
**Direct SQL:** `node scripts/pib-db.js query "UPDATE ..."`
