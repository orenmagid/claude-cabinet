...
// ---------------------------------------------------------------------------
// --unmigrate-memory: restore from backup dir
// ---------------------------------------------------------------------------

/**
 * Reverse of migrateMemoryCmd: restore the 4 backed-up files to their
 * original locations. Reads the backup dir from
 * .ccrc.json.migrated_from_omega.backupDir. Sets state = 'rolled-back'.
 *
 * Does NOT delete migrated topic files at
 * ~/.claude/projects/<slug>/memory/ — the omega DB still has the
 * source data, and the user can keep or manually delete the topic
 * files as they choose. This is the safer asymmetry: restoring is
 * easy, destruction is opt-in.
 */
async function unmigrateMemoryCmd(opts = {}) {
  const p = paths(opts);
  const dryRun = !!opts.dryRun;
  ...
  if (!state) {
    return {
      ok: false,
      reason: 'no-migration-state',
      message: `No migrated_from_omega state found in ${p.ccrcPath}. Nothing to roll back.`,
    };
  }
  if (state.state === 'rolled-back') {
    return {
      ...
      message: `Migration was already rolled back on ${state.rolledBackAt || 'unknown date'}.`,
    };
  }
  if (!state.backupDir || !fs.existsSync(state.backupDir)) {
    return {
      ...
      message: `Backup dir ${state.backupDir || '(none recorded)'} does not exist. Cannot roll back automatically. See manual rollback in ROLLBACK.md if you have a copy.`,
    };
  }

  const restorations = [];
  const targets = [
    { src: 'CLAUDE.md', dest: p.userClaudeMd },
    { src: 'settings.json', dest: p.userSettings },
    { src: 'claude.json', dest: p.userClaudeJson },
    { src: 'project-mcp.json', dest: p.projectMcp },
  ];

  for (const { src, dest } of targets) {
    const backupFile = path.join(state.backupDir, src);
    if (!fs.existsSync(backupFile)) continue;
    if (dryRun) {
      restorations.push(`would restore ${backupFile} → ${dest}`);
      continue;
    }
    ...
    restorations.push(`restored ${dest}`);
  }

  if (!dryRun) {
    state.state = 'rolled-back';
    state.rolledBackAt = nowIso();
    persistState(p, state, false);
  }
  ...
  return {
    ...
    restorations,
    message:
      `Rolled back ${restorations.length} file(s) from ${state.backupDir}.\n` +
      `Topic files at the memory dir were preserved (delete manually if desired).\n` +
      `Omega should be functional again on next session start.`,
  };
}
