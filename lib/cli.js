...
async function run() {
  ...
  if (flags.unmigrateMemory) {
    ...
    if (typeof unmigrateMemoryCmd !== 'function') {
      console.error('  --unmigrate-memory not yet implemented; ships in Phase 4b.');
      process.exit(2);
    }
    const projectDir = path.resolve(flags.targetDir);
    const result = await unmigrateMemoryCmd({ cwd: projectDir });
