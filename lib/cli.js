...
const MODULES = {
  ...
  'planning': {
    ...
    description: 'Structured implementation planning with cabinet critique and execution checkpoints.',
    mandatory: false,
    default: true,
    lean: true,
  ...
  'audit': {
    ...
    description: 'Periodic expert cabinet member analysis with structured triage. Most compute-intensive module.',
    mandatory: false,
    default: true,
    lean: true,
    templates: [
...
function printHelp() {
  ...
  Examples:
    ...
    npx create-claude-cabinet --lean          Session loop + planning + perspectives
...
async function run() {
  ...
  if (firstCorInstall && !flags.yes && !flags.lean && !flags.dryRun) {
    ...
    if (!claudeMdContent.includes('# About Me')) {
      console.log('  This looks like your first time using Claude Cabinet.');
  ...
  console.log('  Setting up Claude Cabinet...\n');
