 ...
 */
async function copyTemplates(src, dest, { dryRun = false, skipConflicts = false } = {}) {
  const results = { copied: [], skipped: [], overwritten: [] };
  await walkAndCopy(src, dest, src, results, dryRun, skipConflicts);
  return results;
}

async function walkAndCopy(srcRoot, destRoot, currentSrc, results, dryRun, skipConflicts) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      await walkAndCopy(srcRoot, destRoot, srcPath, results, dryRun, skipConflicts);
    } else {
